    ...
    return {
      ...
      rules: {
        username: [
          ...
          { min: 1, max: 4, message: '姓名应为1~4位', trigger: 'blur' }
        ],
        ...
        timeOfEntry: [
          { required: 'true', message: '请输入职时间', trigger: 'change' },
          { validator: (rule, value, callback) => {
            // 是否有选定转正时间
            if (this.userInfo.correctionTime) {
              // 入职时间是否大于转正时间
              if (new Date(this.userInfo.correctionTime) < new Date(value)) {
